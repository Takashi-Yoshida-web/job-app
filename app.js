...
// -------------------------------------------------------------------------------------------
// 
// フロントの「Gmail連携」ボタンを押したときに、このURLにアクセスさせてGoogleのログイン画面に飛ばします
...
// -------------------------------------------------------------------
// 
// ユーザーがGoogleで許可を押すと、ここに自動でジャンプしてきます
app.get('/api/auth/google/callback', async (req, res) => {
    try {
        ...
        // 1. 一時的なコードを使って、Googleから「アクセストークン」と「リフレッシュトークン」を貰う
        ...
        // 3. 連携完了画面へリダイレクト（または簡易メッセージ表示）
        // res.send(`
        //     <html>
        //         <body style="font-family: sans-serif; text-align: center; padding-top: 50px;">
        //             <h2 style="color: #28a745;">Gmailとの連携が完了しました！</h2>
...
const Application = sequelize.define('Application',{
    ...
    status:{type: DataTypes.STRING, defaultValue:'応募中'},
    source:{ type: DataTypes.STRING,defaultValue:'その他' },
    interviewDate:{ type: DataTypes.DATE, allowNull: true},
    belongings:{ type: DataTypes.STRING, allowNull: true},
    companyUrl:{ type: DataTypes.STRING, allowNull: true},
    ratingSalary:{type: DataTypes.INTEGER, defaultValue:0},
    ratingHoliday:{type: DataTypes.INTEGER, defaultValue:0},
    deadlineDate:{ type: DataTypes.STRING, allowNull: true}
...
app.get ('/profile', authenticateToken, async (req,res) => {
    try{
        ...
        const user = await User.findByPk(req.user.userId,{
            attributes:['id','email'] //カラムをIDとemailに絞ってデータベースから取得
        });
    ...
    try {
        ...
        for (const msgInfo of messages) {
            ...
            await gmail.users.messages.batchModify({
                userId: 'me',
                requestBody: { // ★「resource」ではなく「requestBody」になっているか確認
...
app.put('/api/applications/:id',authenticateToken, async(req,res) =>{
   try{
    ...
    const {companyName, jobTitle, status, source, interviewDate, belongings, companyUrl, ratingSalary, 
        ratingHoliday, deadlineDate } = req.body;
...
// 
// フロント側から `/api/applications/ranking?sortBy=salary` のようにリクエストを受け取ります
...
// 
// 選考中（不採用・内定以外）で、かつ deadlineDate が入力されているものを期限が近い順に返します
app.get('/api/applications/alerts', authenticateToken, async (req, res) => {
    try {
        ...
        const urgentApplications = await Application.findAll({
            ...
            // deadlineDate は文字列（"2026-06-01"など）でも昇順（ASC）で並べれば日付が近い順になります
